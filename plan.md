...
|-------|-------------|-------|
| **1. Foundation** | `go mod init`, config structs, domain types, service interface | `go.mod`, `internal/config/config.go`, `worktree/types.go`, `worktree/service.go` |
| **2. Git Implementation** | Real git worktree operations, parse porcelain output | `internal/git/worktree.go` |
| **3. Core TUI** | Model struct, `New()` with options, `Init()` loads all worktrees, list rendering grouped by repo | `worktree/model.go`, `worktree/commands.go`, `worktree/messages.go`, `worktree/delegate.go`, `worktree/styles.go`, `worktree/keymap.go` |
